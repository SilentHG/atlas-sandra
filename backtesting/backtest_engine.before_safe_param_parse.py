...
class BacktestEngine:
    ...
    async def _apply_strategy_signals(self, df: pd.DataFrame, strategy_id: str = "") -> pd.DataFrame:
        ...
        Falls back to default EMA crossover only when no generated code is available.
        """
        if not strategy_id:
        ...
        if not row or not row.get("code"):
            logger.warning("[backtest] No generated code found for {}; using default signals", strategy_id)
            return self._compute_signals(df)
        ...
        try:
            ...
            logger.warning("[backtest] Generated strategy code exec failed for {}: {}; using default signals", strategy_id, exc)
            return self._compute_signals(df)
        ...
        try:
            ...
            if not strategy_cls:
                logger.warning("[backtest] No BaseStrategy subclass found for {}; using default signals", strategy_id)
                return self._compute_signals(df)

            # Generated strategy classes currently use one of these constructor styles:
            #   __init__(parameters=None)
            #   __init__(name, symbols)
            #   __init__(symbols)
            #   __init__()
            symbols = [str(df.get("symbol", pd.Series([""])).iloc[0])] if "symbol" in df.columns else [str(row["name"]).split("_")[0].upper()]

            db_params = dict(row.get("parameters") or {})
            ...
            try:
                logger.info(
                    ...
                    strategy_id,
                    pd.Series(signals).astype(str).value_counts(dropna=False).head(20).to_dict()
                )
            except Exception as exc:
                logger.warning("[backtest] Could not log signal distribution for {}: {}", strategy_id, exc)

            if signals is None or len(signals) == 0:
                logger.warning("[backtest] Strategy {} returned empty signals; using default signals", strategy_id)
                return self._compute_signals(df)

            out = df.copy()

            def _signal_to_num(x):
                # Supports numeric, Signal enum, strings, and TradeSignal-like objects.
                if hasattr(x, "signal"):
                    x = x.signal
                if hasattr(x, "value"):
                    x = x.value
                if isinstance(x, str):
                    ...
                    if y == "SELL":
                        return -1.0
                    if y in ("HOLD", "CLOSE", "EXIT", "FLAT", "NONE"):
                        return 0.0
                    try:
                        return float(x)
                    except Exception:
            ...
            logger.info("[backtest] Applied generated strategy signals for {} ({})", strategy_id, row["name"])
            return out

        except Exception as exc:
            logger.warning("[backtest] Generated strategy signal application failed for {}: {}; using default signals", strategy_id, exc)
            return self._compute_signals(df)
